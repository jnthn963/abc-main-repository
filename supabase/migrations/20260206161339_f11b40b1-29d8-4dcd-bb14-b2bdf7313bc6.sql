-- Update lend_capital_atomic to trust p_user_id when called from service role context
-- Service role calls have auth.uid() = NULL, but the edge function already verified the JWT

CREATE OR REPLACE FUNCTION public.lend_capital_atomic(p_user_id UUID, p_amount BIGINT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF auth.uid() IS NOT NULL THEN
    ...
    END IF;
  END IF;
  -- Note: When auth.uid() IS NULL (service role), we trust p_user_id from edge function
