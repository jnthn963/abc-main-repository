...
import { 
  X, FileText, Clock, AlertTriangle, CheckCircle, 
  User, Calendar, Percent, Lock, Unlock, ArrowRight
} from "lucide-react";
...
import { getMemberData } from "@/stores/memberStore";
...
const MyLoansPanel = ({ isOpen, onClose }: MyLoansPanelProps) => {
  ...
  // Load and subscribe to loans
  useEffect(() => {
    const loadLoans = () => {
      const allLoans = getLoans();
      // Filter loans where current user is the borrower
      const borrowerLoans = allLoans.filter(l => l.borrowerId === member.id);
      setMyLoans(borrowerLoans);
    };

    loadLoans();
    const unsubscribe = subscribeLoanStore(loadLoans);
    return () => unsubscribe();
  }, [member.id]);
  ...
  const handleRepaymentComplete = (loanId: string) => {
    // Call the repayLoan function from the store
    const result = repayLoan(loanId, member.id);
    if (!('error' in result)) {
      // Refresh loans
      const allLoans = getLoans();
      const borrowerLoans = allLoans.filter(l => l.borrowerId === member.id);
  ...
  const activeFundedLoans = myLoans.filter(l => l.status === 'funded');
  const pendingLoans = myLoans.filter(l => l.status === 'open');
  const completedLoans = myLoans.filter(l => l.status === 'repaid' || l.status === 'defaulted');
